...
public class TeamLineupServiceImpl implements TeamLineupService {
    ...
    public List<Batter> getAvailableBattersWithStats(String teamName) {
        logger.info("[DEBUG] getAvailableBattersWithStats 호출됨. teamName: {}", teamName);
        try {
            String dbTeam = resolveDbTeamName(teamName);
            logger.info("[DEBUG] Hitter DB 조회용 팀명: {}", dbTeam);
            if (teamLineupMapper != null) {
                List<Batter> batters = teamLineupMapper.findAvailableBattersByTeam(dbTeam);
                logger.info("[DEBUG] DB에서 찾은 타자 수: {}", batters == null ? 0 : batters.size());
                if (batters != null && !batters.isEmpty()) {
                    
                    logger.info("팀 {}의 타자 성적 데이터 {}명을 DB에서 조회하고 타율을 계산했습니다.", teamName, batters.size());
                    return batters;
                }
            }
        } catch (Exception e) {
    ...
    public List<Pitcher> getAvailablePitchersWithStats(String teamName) {
        logger.info("[DEBUG] getAvailablePitchersWithStats 호출됨. teamName: {}", teamName);
        try {
            String dbTeam = resolveDbTeamName(teamName);
            logger.info("[DEBUG] Pitcher DB 조회용 팀명: {}", dbTeam);
            if (teamLineupMapper != null) {
                List<Pitcher> pitchers = teamLineupMapper.findAvailablePitchersByTeam(dbTeam);
                logger.info("[DEBUG] DB에서 찾은 투수 수: {}", pitchers == null ? 0 : pitchers.size());
    ...
    public List<Batter> getAllBattersWithStats(String sortBy) {
        logger.info("[DEBUG] getAllBattersWithStats 호출됨. sortBy: {}", sortBy);
        try {
            if (teamLineupMapper != null) {
                List<Batter> batters = teamLineupMapper.findAllBatters(sortBy);
                logger.info("[DEBUG] DB에서 찾은 모든 타자 수: {}", batters == null ? 0 : batters.size());
                if (batters != null && !batters.isEmpty()) {
                    return batters;
                }
            }
        } catch (Exception e) {
            logger.warn("모든 타자 성적 데이터 DB 조회 실패: {}", e.getMessage());
        }
        return java.util.Collections.emptyList();
    }
    ...
    public List<Pitcher> getAllPitchersWithStats(String sortBy) {
        logger.info("[DEBUG] getAllPitchersWithStats 호출됨. sortBy: {}", sortBy);
        try {
            if (teamLineupMapper != null) {
                List<Pitcher> pitchers = teamLineupMapper.findAllPitchers(sortBy);
                logger.info("[DEBUG] DB에서 찾은 모든 투수 수: {}", pitchers == null ? 0 : pitchers.size());
                if (pitchers != null && !pitchers.isEmpty()) {
                    return pitchers;
                }
            }
        } catch (Exception e) {
            logger.warn("모든 투수 성적 데이터 DB 조회 실패: {}", e.getMessage());
        }
        return java.util.Collections.emptyList();
    }
    ...
    public List<TeamStats> getAllTeamStats() {
        logger.info("[DEBUG] getAllTeamStats 호출됨.");
        try {
            if (teamLineupMapper != null) {
                List<TeamStats> teamStats = teamLineupMapper.findAllTeamStats();
                logger.info("[DEBUG] DB에서 찾은 모든 팀 수: {}", teamStats == null ? 0 : teamStats.size());
                if (teamStats != null && !teamStats.isEmpty()) {
                    return teamStats;
                }
            }
        } catch (Exception e) {
            logger.warn("모든 팀 성적 데이터 DB 조회 실패: {}", e.getMessage());
        }
        return java.util.Collections.emptyList();
    }
