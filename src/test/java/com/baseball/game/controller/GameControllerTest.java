...
class GameControllerTest {
    ...
    void createGame_ShouldReturnCreatedGameDto_WhenRequestIsValid() throws Exception {
        ...
        given(gameService.createGame(anyString(), anyString(), anyInt(), anyBoolean())).willReturn(sampleGameDto);
    ...
    @Test
    @DisplayName("투구 API - 성공")
    void pitcherThrow_ShouldReturnSuccessResponse_WhenRequestIsValid() throws Exception {
        // Given: 유효한 투구 요청
        GameActionRequest request = new GameActionRequest();
        request.setPitchType("Fastball");
    ...
    @Test
    @DisplayName("타격 API - 성공")
    void batterSwing_ShouldReturnSuccessResponse_WhenRequestIsValid() throws Exception {
        // Given
        GameActionRequest request = new GameActionRequest();
        request.setDecisionToSwing(true);
    ...
    void batterSwing_ShouldThrowValidationException_WhenDecisionToSwingIsNull() throws Exception {
        ...
        GameActionRequest request = new GameActionRequest();
        request.setDecisionToSwing(null);
        request.setTiming(0.5);
    ...
    @Test
    @DisplayName("컴퓨터 턴 진행 API - 성공")
    void playComputerTurn_ShouldReturnSuccessResponse() throws Exception {
        // Given
        String computerTurnResult = "컴퓨터가 뜬공으로 아웃되었습니다.";
        given(gameService.playComputerTurn(gameId)).willReturn(computerTurnResult);
        given(gameService.getGame(gameId)).willReturn(sampleGameDto);

        // When
        ResultActions resultActions = mockMvc.perform(post("/api/baseball/game/{gameId}/computer-turn", gameId)
                .contentType(MediaType.APPLICATION_JSON));

        // Then
        resultActions.andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("컴퓨터의 턴이 성공적으로 진행되었습니다: " + computerTurnResult))
                .andDo(print());
    }
