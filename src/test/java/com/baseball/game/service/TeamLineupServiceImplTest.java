...
class TeamLineupServiceImplTest {
    ...
    @Mock
    private com.baseball.game.mapper.BatterMapper batterMapper;

    @Mock
    private com.baseball.game.mapper.PitcherMapper pitcherMapper;
    ...
    void saveCustomLineup_and_getCustomLineup() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            pos.setPosition(i + 1);
            lineup.add(pos);
            // Mock batterMapper.findByName for each batter
            when(batterMapper.findByName(batters[i])).thenReturn(new Batter(batters[i], teamName));
        }
        // 선발 투수
        ...
        // Mock pitcherMapper.findByName for the pitcher
        when(pitcherMapper.findByName("박세웅")).thenReturn(new Pitcher("박세웅", teamName));
    ...
    void saveCustomLineup_lessThan9Batters() {
        ...
        // Mock findByName for specific players
        when(batterMapper.findByName("황성빈")).thenReturn(new Batter("황성빈", teamName));
        when(pitcherMapper.findByName("박세웅")).thenReturn(new Pitcher("박세웅", teamName));
        // Ensure other names return null for the wrong mapper
        when(batterMapper.findByName(argThat(name -> !name.equals("황성빈")))).thenReturn(null);
        when(pitcherMapper.findByName(argThat(name -> !name.equals("박세웅")))).thenReturn(null);

        // 8명만 추가
        for (int i = 0; i < 8; i++) {
            CustomLineupRequest.LineupPosition pos = new CustomLineupRequest.LineupPosition();
            pos.setPlayerName("황성빈"); // Player name doesn't matter as findByName is mocked for anyString()
            pos.setPosition(i + 1);
            lineup.add(pos);
        }
        // 투수
    ...
    void getBatterAndPitcherByName() {
        // Mocking behavior for findByName
        Batter mockBatter = new Batter("황성빈", "Giants");
        Pitcher mockPitcher = new Pitcher("박세웅", "Giants");

        when(batterMapper.findByName("황성빈")).thenReturn(mockBatter);
        when(pitcherMapper.findByName("박세웅")).thenReturn(mockPitcher);
